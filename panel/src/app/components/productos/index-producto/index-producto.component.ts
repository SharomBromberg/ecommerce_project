    ...
    this._productoService.getProductos(filtro, this.token).subscribe(
      response => {
        ...
        if (response.data != undefined) {
          this.productos = response.data; // se muestra en la interfaz grafica, ósea el html 
          this.const_productos = this.productos; // respaldo de datos para que no se alteren los datos originales
        } else {
